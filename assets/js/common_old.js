	...
	if (validateuserName() && validateuserLName() && validateuseremail() && validateuserpass() && validateuserDName() && validateprogram() && validatepdate())
	{
		...
		$.ajax({
			...
			success: function (d) {
				//alert(d);
				$("#msg_txt").show();
				$("#msg_txt").html("Student details added successfully");
				setTimeout("$('#msg_txt').hide()", 10000);
				setTimeout("$('#msg_txt').html('')", 10000);
				//xmlhttp.open("GET","http://api.gradmoz.com:9090/plugins/userService/userservice?type=add&secret=ygw9D4op&username=100sandyccc&password=1234567890",true);
				//xmlhttp.send();
